...
async function test() {
  const { data, error } = await supabase.from('siswa').select('*').limit(1);
  console.log('siswa error:', error?.message);
  const { data: d2, error: e2 } = await supabase.from('Siswa').select('*').limit(1);
  console.log('Siswa error:', e2?.message);
}
test();
