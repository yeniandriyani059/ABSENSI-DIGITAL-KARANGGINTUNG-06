...
async function test() {
  const { data, error } = await supabase.from('siswa').insert([{ nisn: '124', nama: 'Test2', kelas: '2' }]).select();
  console.log('siswa id type:', typeof data?.[0]?.id);
  console.log('siswa id:', data?.[0]?.id);
}
test();
